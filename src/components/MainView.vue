...
import {
  ShowdownsEditor,
  Showdowns,
} from "@jhuix/vue-showdowns-editor";
...
export default {
  ...
  methods: {
    ...
    onMdChange(doc) {
      this.mdDoc = doc;
      this.$store.dispatch("storeFileInfo", doc);
    },
    onMdScroll(cm) {
      if (this.previewer) {
        try {
  ...
  mounted() {
    ...
    this.$nextTick(() => {
      if (this.hasToolbar) {
        ...
        if (this.titletool)
          this.titleToolWidth = this.titletool.$el.clientWidth;
        ...
        if (this.aligntool)
          this.alignToolWidth = this.aligntool.$el.clientWidth;
